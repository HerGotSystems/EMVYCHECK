...
export class SceneTimelineController {
  ...
  handleActions() {
    for (const action of this.timeline.actions) {
      if (action.time > this.time) continue;
...
export function bindTimelineToScene({ scene, audio, timelineData }) {
  ...
  if (audio) {
    ...
    audio.addEventListener('seeked', () => {
      controller.firedActions.clear();
      controller.activeSectionIndex = -1;
      controller.lastTime = audio.currentTime;

      for (const action of controller.timeline.actions) {
        if (action.time <= audio.currentTime) controller.firedActions.add(action.id);
      }
    });
  } else {
