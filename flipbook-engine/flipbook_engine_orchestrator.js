...
export class FlipbookEngineOrchestrator {
  ...
  installSceneFrameLoop() {
    const originalTick = this.scene.tick?.bind(this.scene);
    if (!originalTick || this.scene.__orchestratorWrapped) return;

    const engine = this;
    this.scene.tick = function wrappedTick(now) {
      ...
      for (const fn of engine.frameHandlers) {
        try {
          fn({
            now,
            deltaMs,
    ...
    this.scene.__orchestratorWrapped = true;
  }
